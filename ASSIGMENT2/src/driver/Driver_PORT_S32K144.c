...
static GPIO_Type * const GPIO_BASE[] = IP_GPIO_BASE_PTRS;   /* Base address cho GPIOA..E */
 ...
 */
static void PORT_HandleIRQ(uint32_t port) {
  uint32_t flags = PORT_BASE[port]->ISFR;  /* Lấy cờ ngắt */

  for (uint32_t i = 0; i < 32; i++) {
    if (flags & (1UL << i)) {
      uint32_t pin = port * 32U + i;

      /* Clear flag */
      PORT_BASE[port]->ISFR = (1UL << i);

      /* Nếu có callback thì gọi */
      if (port_cb[pin]) {
        ARM_PORT_EVENT event;

        /* Xác định mức hiện tại để báo rising/falling */
        if (GPIO_BASE[port]->PDIR & (1UL << i)) {
          event = ARM_PORT_EVENT_RISING_EDGE;
        } else {
          event = ARM_PORT_EVENT_FALLING_EDGE;
        }

        port_cb[pin](pin, event);
      }
    }
  }
}
...
void PORTA_IRQHandler(void) { PORT_HandleIRQ(0); }
void PORTB_IRQHandler(void) { PORT_HandleIRQ(1); }
void PORTC_IRQHandler(void) { PORT_HandleIRQ(2); }
void PORTD_IRQHandler(void) { PORT_HandleIRQ(3); }
void PORTE_IRQHandler(void) { PORT_HandleIRQ(4); }
